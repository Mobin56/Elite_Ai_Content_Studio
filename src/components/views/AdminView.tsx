...
import { 
  ...
  Plus, Trash2, Key, UserPlus, Gift, Database, Lock, Unlock
} from 'lucide-react';
...
export default function AdminView() {
  ...
  // Authentication state for the active session
  ...
  const handleAllocate = () => {
    ...
    alert(`Successfully allocated ${creditAmount} credits to ${target?.name || 'User'}`);
  };
  ...
  if (!isAdminAuthenticated) {
    return (
      <div className="max-w-md mx-auto my-16 p-8 rounded-2xl glass-card text-center space-y-6 animate-in zoom-in-95 duration-200">
        <div className="w-16 h-16 rounded-2xl bg-rose-500/10 flex items-center justify-center text-rose-400 mx-auto glow-secondary">
          <Lock className="w-8 h-8" />
        </div>
        <div className="space-y-2">
          <h3 className="text-xl font-bold tracking-tight">Super Admin Gateway</h3>
          <p className="text-xs text-muted-foreground max-w-xs mx-auto">
            This workspace panel is restricted to the Super Admin. Enter the security key to authenticate.
          </p>
        </div>

        <form onSubmit={handleAuthenticate} className="space-y-4">
          <div className="space-y-1.5 text-left">
            <label className="text-xs font-semibold text-slate-400">Admin Password Key</label>
            <input
              type="password"
              value={inputPassword}
              onChange={(e) => setInputPassword(e.target.value)}
              className="w-full px-4 py-2.5 text-sm rounded-lg glass-input text-foreground font-mono"
              placeholder="••••••••••••"
              autoFocus
            />
            {authError && (
          ...
          <button
            type="submit"
            className="w-full py-2.5 bg-rose-600 hover:bg-rose-500 text-white rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-lg shadow-rose-600/10"
          >
        ...
        <p className="text-[9px] text-slate-500">
          Default Super Admin Password is <span className="font-mono bg-slate-900 px-1 py-0.5 rounded">admin123</span>
        </p>
      </div>
    );
  }
      ...
      <div className="border-b border-[var(--glass-border)] pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 border border-[var(--glass-border)] text-xs font-bold text-slate-400 hover:text-foreground transition-all cursor-pointer"
        ...
        <div className="p-5 rounded-2xl glass-card space-y-4 h-max">
          ...
          <div className="space-y-4 text-xs">
            ...
            <button
              ...
              className="w-full py-2.5 bg-rose-600 hover:bg-rose-500 text-white rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            >
              Grant Credits
            </button>
          </div>
